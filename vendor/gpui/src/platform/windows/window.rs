...
pub struct WindowsWindowState {
    ...
    pub hovered: bool,
    /// PERCH PATCH: the renderer was rebuilt after a device loss and is
    /// refusing to draw until a frame is rendered fresh. The next `draw_window`
    /// has to be a forced render so it bypasses the view cache, which would
    /// otherwise replay tiles belonging to the device that went away.
    pub force_render_pending: bool,
...
impl WindowsWindowState {
    fn new(
        ...
        let hovered = false;
        // PERCH PATCH: see `WindowsWindowState::force_render_pending`.
        let force_render_pending = false;
